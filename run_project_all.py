...
@click.command()
@click.option('--network', 'network_pkl', help='Network pickle filename', default ='./checkpoints/00012-target-face256/network-snapshot-002400.pkl')
...
def run_projection(
    ...
    dic = '/data/home/uss00022/lelechen/data/Facescape/textured_meshes'
    # dic = '/mnt/Backup/lele/Facescape/textured_meshes'
    ids = os.listdir(dic)
    ids = sorted(ids)
    print (len(ids))
    exps = get_exp()
    for id in ids[350:400]:
        print (id)
        for exp in exps:
            if os.path.exists(os.path.join(dic, id, 'models_reg', exp + '.npz')):
                continue
            try:
                target_fname = os.path.join(dic, id, 'models_reg', exp + '.jpg')
                ...
                if save_video:
                    video = imageio.get_writer(f'{outdir}/{id}_{exp}.mp4', mode='I', fps=10, codec='libx264', bitrate='16M')
                    print (f'Saving optimization progress video "{outdir}/{id}_{exp}.mp4"')
                ...
                target_pil.save(f'{outdir}/{id}_{exp}_target.png')
                projected_w = projected_w_steps[-1]
                synth_image = G.synthesis(projected_w.unsqueeze(0), noise_mode='const')
                synth_image = (synth_image + 1) * (255/2)
                synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
                PIL.Image.fromarray(synth_image, 'RGB').save(f'{outdir}/{id}_{exp}_proj.png')
