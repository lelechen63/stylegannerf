...
def run_projection(
    ...
    # target_pil = PIL.Image.open(target_fname).convert('RGB')#[500:2500, 1019 : 3019]
    # w, h = target_pil.size
    # s = min(w, h)
    # # target_pil = target_pil.crop(((w - s) // 2, (h - s) // 2, (w + s) // 2, (h + s) // 2))
    # target_pil = target_pil.resize((G.img_resolution, G.img_resolution), PIL.Image.LANCZOS)
    # target_uint8 = np.array(target_pil, dtype=np.uint8) * mask
    # target_uint8 = target_uint8.astype(np.uint8)
